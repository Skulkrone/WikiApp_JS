...
async function wikiApiCall(searchInput) {
  try {
    ...
    // si erreur 404 et qu'ont veut la voir :
    if (!response.ok) {
      throw new Error(`${response.status}`);
    }
    // on récupérer les données qui sont traduites au format json pour pouvoir ensuite les récupérer et affichervvvsv
    const data = await response.json();
    // console.log(data);

    // fonction qui va créer des cartes pour présenter les données récupérées au format html
...
function createCards(data) {
  ...
  data.forEach((el) => {
    ...
    card.innerHTML = `
      ...
      <br>
    `;
    // rajoute un enfnat à resultsDisplay
